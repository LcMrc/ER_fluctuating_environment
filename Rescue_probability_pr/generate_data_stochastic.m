...
mu = 1e-2;               % Mutation probability upon division
sigma = 1;               % Standard deviation of the stochastic environmental fluctuations

for i = length(tau) : -1 : 1
    
    NMlist(i, :) = Gillespie_stochastic(Nit, bWF, bWH, dWF, dWH, NW0, bMF, bMH, dMF, dMH, NM0, K, tau(i), mu, sigma);
...
fname = sprintf('pr_vs_tau_stochastic_mu%d_sigma%d.mat', mu, sigma);
save(fname)

% Data plotting

fig = figure('Name','Rescue probability - Stochastic environmental fluctuations','NumberTitle','off');
hold on  
